...
def reduce_clause(all_clause_pairs):
    ...
    -2, 3
    -3, 2
    because variable 1 has no negation in all clauses, we just set it to be true,
...
# -------------------------------------------------------------------------------------------------
def Papadimitriou_Local_Search(clause_pairs):
    """
...
if __name__ == "__main__":
    ...
    for input_idx in range(1, len(sys.argv)):
        all_clause_pairs = set()
        with open(sys.argv[input_idx]) as input_file_obj:
            print ("################  Processing input: " + sys.argv[input_idx])
            next(input_file_obj)
        ...
        print ("reduced_clause_pairs count={}\n".format(len(reduced_clause_pairs)))
        answers.append( Papadimitriou_Local_Search(reduced_clause_pairs) )

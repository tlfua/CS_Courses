...
class Router(object):
    ...
    def find_next_hop(self, dest_addr):
        ...
        max_prefixlen = 0
        ret_next_hop_addr, ret_next_hop_port = None, None
        for net_addr, value in self.forwarding_table.items():
            mask, next_hop_addr, next_hop_port = value
            ...
            if (int(net_addr) & int(dst_addr)) == int(net_addr):
                ...
                if prefixlen > max_prefixlen:
                    ...
                    ret_next_hop_port = next_hop_port
        if not ret_next_hop_addr or not ret_next_hop_port:
            print ("Error: can not find next hop")
            os._exit(1)
        return ret_next_hop_addr, ret_next_hop_port

    def forward_packet(self, port, next_hop_ip, pkt, change_ip_src=False):
        ...
        intf = self.net.interface_by_name(port)
        ...
        if next_hop_ip in self.arp_table:
            ...
            if next_hop_ip not in self.arp_requests:
                self.arp_requests[next_hop_ip] = [port, arprequest,
                                                  time.time(), 1]
                ...
                print ("fwd ARP pkt")
                self.net.send_packet(port, arprequest)
...
def main(net):
    ...
    '''
    print ("myswitch_part2: new version")

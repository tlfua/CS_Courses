...
class Router(object):
    ...
    def find_next_hop(self, dest_addr):
        ...
        for net_addr, value in self.dynamic_forwarding_table.items():
        ...
        if ret_next_hop_addr == None and ret_next_hop_dev == None:
            ...
            try:
                ...
                if eth_payload.ethertype == EtherType.ARP:  # ARP
                    ...
                    if dst_ip not in self.ips and (next_hop_ip and next_hop_dev):
                        ...
                        log_debug("self.arp_table is {}\n".format(str(self.arp_table.keys())))
                ...
                elif eth_payload.ethertype == EtherType.SLOW:
                    drm = pkt.get_header(DynamicRoutingMessage)
                    prefix = drm.advertised_prefix

                    mask = drm.advertised_mask
                    next_hop = drm.next_hop

                    if len(self.dynamic_forwarding_table) >= 5:
                    ...
                    log_debug("dynamic table is {}\n".format(self.dynamic_forwarding_table))

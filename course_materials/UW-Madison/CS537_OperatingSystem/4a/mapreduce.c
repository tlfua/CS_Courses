...
pthread_mutex_t mutex_emit = PTHREAD_MUTEX_INITIALIZER;
...
typedef struct _ReducerWrap {
    Reducer reducer;
    Vector* keys;
    int partition_number;
} ReducerWrap;

void* threadCallMultipleReducer(void* reducerwrap)
{
    Reducer reducer = ((ReducerWrap*)reducerwrap)->reducer;
    Vector* keys = ((ReducerWrap*)reducerwrap)->keys;
    int partition_number = ((ReducerWrap*)reducerwrap)->partition_number;
    void* elem;
    for (int i=0; i<keys->size(keys); i++){
        keys->get(keys, i, &elem);// get key
        reducer((char*)elem, Get, partition_number);
    }
    return 0;
}
...
{
    hashmap = HashMapInit();// (char*, Vector*) pair
    ...
    // HashMapSetCleanValue(key2elemindex, CleanObject); // no need to clean value, think why.
    ...
    // printf ("hashmap size = %d\n", hashmap->size(hashmap)); // print

    Vector* sortedkeys = VectorInit(32);// vector of (char*)
    void* key;
    ...
    while ((ptr_pair = hashmap->next(hashmap)) != NULL){
        ...
        sortedkeys->push_back(sortedkeys, (void*)key);
        key2elemindex->put(key2elemindex, (void*)strdup(key), (void*)(intptr_t)0);
        vector->sort(vector, CompareWord);
    }
    sortedkeys->sort(sortedkeys, CompareWord);

    Vector* reducerkeys = VectorInit(32);
    VectorSetClean(reducerkeys, CleanVector);
    Vector* keys;
    for (int i=0; i<num_reducers; i++){
        keys = VectorInit(32);
        reducerkeys->push_back(reducerkeys, (void*)keys);
    }
    for (int i=0; i<sortedkeys->size(sortedkeys); i++){
        sortedkeys->get(sortedkeys, i, &key);// get key
        int partition_number = partition((char*)key, num_reducers);
        reducerkeys->get(reducerkeys, partition_number, &elem);// get vector keys
        ((Vector*)elem)->push_back((Vector*)elem, (void*)key);
    }
    
    // for (int i=0; i<reducerkeys->size(reducerkeys); i++){   // print
    //     reducerkeys->get(reducerkeys, i, &elem);
    //     printf ("%d th partition has %d keys\n", i, ((Vector*)elem)->size((Vector*)elem));
    // }

    Vector* reducerwraps = VectorInit(32);
    VectorSetClean(reducerwraps, CleanObject);
    for (int i=0; i<num_reducers; i++){
        ReducerWrap* reducerwrap = (ReducerWrap*)malloc(sizeof(ReducerWrap));
        reducerwrap->reducer = reducer;
        reducerkeys->get(reducerkeys, i, &elem);// get keys
        reducerwrap->keys = (Vector*)elem;
        reducerwrap->partition_number = i;
        reducerwraps->push_back(reducerwraps, (void*)reducerwrap);
    }

    // reducer threads
    pthread_t reducerthreads[num_reducers];
    for (int i=0; i<num_reducers; i++){
        reducerwraps->get(reducerwraps, i, &elem);
        pthread_create(&reducerthreads[i], NULL, threadCallMultipleReducer, (void*)elem);
    }
    for (int i=0; i<num_reducers; i++){
    ...
    VectorDeinit(sortedkeys);

    VectorDeinit(mapperfiles);
    VectorDeinit(mapperwraps);

    VectorDeinit(reducerkeys);
    VectorDeinit(reducerwraps);
}

void MR_Emit(char *key, char *value)
{
    pthread_mutex_lock(&mutex_emit);
    ...
    pthread_mutex_unlock(&mutex_emit);
}

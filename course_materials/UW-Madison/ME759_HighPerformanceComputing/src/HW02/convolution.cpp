              ...
              const float *mask, std::size_t m)
{
  ...
  // construct image_pad
  float *image_pad = new (std::nothrow) float[(n + 2 * p) * (n + 2 * p)];
  ...
  for (std::size_t i = 0; i < n * n; i++)
  {
    ...
    image_pad[(n + 2 * p) * (image_row_index + p) + (image_col_index + p)] =
        image[i];
  }

  // convolution
  for (std::size_t output_row_index = 0; output_row_index < n;
       output_row_index++)
  {
    for (std::size_t output_col_index = 0; output_col_index < n;
         output_col_index++)
    {
      ...
      for (std::size_t i = 0; i < m * m; i++)
      {
        ...
        output[n * output_row_index + output_col_index] +=
            image_pad[(n + 2 * p) * (output_row_index + mask_row_index) +

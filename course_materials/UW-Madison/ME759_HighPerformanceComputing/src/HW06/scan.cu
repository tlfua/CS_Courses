...
// 1.
__global__ void hillis_steele(const float *g_idata, float *g_odata,
                              unsigned int n) {
  ...
  for (unsigned int offset = 1; offset < threads_per_block; offset *= 2) {
    ...
    if (tid >= offset) {
      temp[pout * threads_per_block + tid] =
          temp[pin * threads_per_block + tid] +
          temp[pin * threads_per_block + tid - offset];
    } else {
      temp[pout * threads_per_block + tid] =
          temp[pin * threads_per_block + tid];
    }
...
// 2.
__global__ void extract_sums(const float *g_elems, float *g_sums,
                             unsigned int step) {
  unsigned int tid = threadIdx.x;
  g_sums[tid] =
      g_elems[step * tid + (step - 1)]; // the last elem of a block in step 1.
}

// 4.
__global__ void add_scanned_sums(float *g_elems, float *g_sums,
                                 unsigned int n) {
  unsigned int tid = threadIdx.x;
  unsigned int bid = blockIdx.x;
  unsigned int threads_per_block = blockDim.x;
...
__host__ void shift(const float *g_elems, float *out, unsigned int n)
{
    ...
    cudaMemcpy(out + 1, g_elems, (n - 1) * sizeof(float),
...
__host__ void scan(const float *in, float *out, unsigned int n,
  ...
  hillis_steele<<<blocks, threads_per_block,
                  2 * threads_per_block * sizeof(float)>>>(g_elems, g_elems, n);
  ...
  */
  // 3. scan sums
  hillis_steele<<<1, threads_per_block,
                  2 * threads_per_block * sizeof(float)>>>(g_sums, g_sums,
                                                           blocks - 1);
  cudaDeviceSynchronize();
  /*
